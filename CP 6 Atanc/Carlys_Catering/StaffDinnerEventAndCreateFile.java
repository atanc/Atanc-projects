...
//variables
    ...
    /**
     * Writes data to file. Uses Public user because all Windows PCs should have it.
     * @param events
     */
    public static void writeDataFile(GraduationEvent[] events)
    {	
    	String comma = ",";
    	String s;
    	try { 
    		  // Specify the filename and path
    	      File fileObj = new File("C://Users//Public//GraduationEvents.txt"); 
    	      ...
    	      if (fileObj.createNewFile()) {  
    	        ...
    	        FileWriter RentalWriter = new FileWriter("C://Users//Public//GraduationEvents.txt");
    	...
    	try {  
    	      ...
    	      FileWriter RentalWriter = new FileWriter("C://Users//Public//GraduationEvents.txt");
    	      //for loop to precess each event 
    	      for(int x = 0; x < events.length; ++x)
              { 
    	    	   // Accept a string  
                  s = events[x].getEventNumber() + comma + events[x].getEventTypeID() + comma + events[x].getGuests() +
                		comma + events[x].getPriceForEvent() + System.getProperty("line.separator");
