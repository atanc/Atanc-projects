...
public class Rental {
    ...
    private void setHoursAndMinutes(int minutes) {
        ...
        basePrice = lesson + equipment.getSurchargeFee();
...
/**
 * Sets the equipment type ID depending on the selection by the user.
 * @param equipmentTypeId
 */
    public void setEquipment(int equipmentTypeId) {
        if(equipmentTypeId < 0 || equipmentTypeId > 10) {
            this.equipment = new EquipmentWithoutLesson(9);
        } else if (equipmentTypeId >= 0 && equipmentTypeId < 6){
            this.equipment = new EquipmentWithLesson(equipmentTypeId);
        } else {
            this.equipment = new EquipmentWithoutLesson(equipmentTypeId);
        }
    }
}
